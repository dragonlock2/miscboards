...
static char const* desc_strings[] = { // keep <= 127 chars due to encoding
    ...
    [static_cast<size_t>(usb_string_desc::PRODUCT)]       = "lpc1768_poe",
...
/* public functions */
void usb_init(void) {
    NVIC_SetVector(USB_IRQn, reinterpret_cast<uint32_t>(usb_handler));
    NVIC_SetPriority(USB_IRQn, configMAX_SYSCALL_INTERRUPT_PRIORITY >> (8 - __NVIC_PRIO_BITS));

    Chip_IOCON_PinMuxSet(LPC_IOCON, 0, 29, IOCON_MODE_INACT | IOCON_FUNC1);
    Chip_IOCON_PinMuxSet(LPC_IOCON, 0, 30, IOCON_MODE_INACT | IOCON_FUNC1);
    Chip_IOCON_PinMuxSet(LPC_IOCON, 2,  9, IOCON_MODE_INACT | IOCON_FUNC1);
    // Chip_IOCON_PinMuxSet(LPC_IOCON, 1, 30, IOCON_MODE_INACT | IOCON_FUNC2);
    Chip_USB_Init();
    const uint32_t clk_en = 0x12; // AHB + Device
